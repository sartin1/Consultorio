...
class admin_home extends fs_controller
{
   ...
   {
      parent::__construct(__CLASS__, 'Páginas', 'admin', TRUE, TRUE, TRUE);
   }
      ...
      else if( isset($_POST['modpages']) )
      {
         ...
         foreach($this->all_pages() as $p)
         {
            if( !$p->exists ) /// la página está en la base de datos pero ya no existe el controlador
            {
               if( $p->delete() )
               {
                  $this->new_message('Se asignaron las paginas correctamente, las que no han sido activadas es porque solo pueden ser visualizadas por programadores');
               }
            }
