...
class cliente extends fs_model
{
   ...
   public function __construct($c=FALSE)
   {
      ...
      if($c)
      {
         ...
         $this->pnacimiento = $this->no_html ($c['pnacimiento']);
         $this->egestacional = $this->no_html ($c['egestacional']);
         $this->antpre = $this->no_html ($c['antpre']);
                  $this->sexo = $this->no_html ($c['sexo']);
                           $this->fnac = date('d-m-Y', strtotime($c['fnac']));
                                    $this->edad = $this->no_html ($c['edad']);
         $this->antfam = $this->no_html ($c['antfam']);
         $this->antper = $this->no_html ($c['antper']);
      ...
      else
      {
         ...
         $this->pnacimiento = NULL;
         $this->egestacional = NULL;
         $this->antpre = NULL;
         $this->sexo = NULL;
         $this->fnac = date('d-m-Y');
         $this->edad = NULL;
         $this->antfam = NULL;
         $this->antper = NULL;
   ...
   public function test()
   {
      ...
      $this->observaciones = $this->no_html($this->observaciones);
       $this->pnacimiento = $this->no_html($this->pnacimiento);
        $this->egestacional = $this->no_html($this->egestacional);
         $this->antpre = $this->no_html($this->antpre);
         $this->edad = $this->no_html($this->edad);
         $this->sexo = $this->no_html($this->sexo);
         $this->fnac = $this->no_html($this->fnac);
          $this->antfam = $this->no_html($this->antfam);
           $this->antper = $this->no_html($this->antper);
      ...
      {
         $this->new_error_msg("Código de paciente no válido.");
      }
      else if( strlen($this->nombre) < 1 OR strlen($this->nombre) > 100 )
      {
         $this->new_error_msg("Nombre de paciente no válido.");
      }
      else if( strlen($this->razonsocial) < 1 OR strlen($this->razonsocial) > 100 )
      {
         $this->new_error_msg("Razón social del paciente no válida.");
      }
      else
         $status = TRUE;
      ...
      if( $this->test() )
      {
         ...
         if( $this->exists() )
         {
            $sql = "UPDATE ".$this->table_name." SET nombre = ".$this->var2str($this->nombre)
                    ...
                    .", pnacimiento = ".$this->var2str($this->pnacimiento)
                    .", egestacional = ".$this->var2str($this->egestacional)
                    .", antpre = ".$this->var2str($this->antpre)
                    .", antfam = ".$this->var2str($this->antfam)
                    .", sexo = ".$this->var2str($this->sexo)
                    .", fnac = ".$this->var2str($this->fnac)
                    .", edad = ".$this->var2str($this->edad)
                    .", antper = ".$this->var2str($this->antper)
         ...
         else
         {
            $sql = "INSERT INTO ".$this->table_name." (codcliente,nombre,razonsocial,cifnif,telefono1,
               ...
               fechaalta,observaciones,edad,sexo,fnac,pnacimiento,egestacional,antpre,antfam,antper,regimeniva,recargo) VALUES (".$this->var2str($this->codcliente)
                    ...
                    .",".$this->var2str($this->observaciones)
                       .",".$this->var2str($this->pnacimiento)
                          .",".$this->var2str($this->egestacional)
                             .",".$this->var2str($this->antpre)
                                .",".$this->var2str($this->antfam)
                                   .",".$this->var2str($this->antper)
                                   .",".$this->var2str($this->sexo)
                                   .",".$this->var2str($this->fnac)
                                   .",".$this->var2str($this->edad)
